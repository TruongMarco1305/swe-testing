...
\begin{table}[ht]
    ...
    \begin{filelist}
        ...
        \item NFR-2: Security Testing (\texttt{requests}-based passive probes)
    \end{filelist} & 20\% \\ \hline

    2 & 2352787 & Trương Gia Kỳ Nam &
    \begin{filelist}
       \item Feature 006: Teacher Sets Up a Quiz -- Level 1 + Level 2 automation
       \item NFR-1: Security Testing (\texttt{requests}-based passive probes)
...
\begin{itemize}
    ...
    \item \textbf{Non-functional testing} -- six Python files implementing three complementary NFR techniques (performance, security, accessibility), each file scoped to one of the six functional features.
...
Security probes & \texttt{requests} 2.31 (passive HTTP probes, no daemon) \\ \hline
...
|   |-- TC-001.py                    # Add User       -> Locust + requests (sec)
|   |-- TC-002.py                    # Create Course  -> Locust + axe-core (a11y)
|   |-- TC-003.py                    # Create Assign  -> requests (sec) + axe
|   |-- TC-004.py                    # Grade Student  -> Locust + requests (sec)
|   |-- TC-005.py                    # Calendar Event -> Locust + axe-core (a11y)
|   `-- TC-006.py                    # Quiz Setup     -> requests (sec) + axe
|
...
\textbf{Role switch:} \texttt{setUpClass()} navigates to \texttt{/course/switchrole.php?id=1\&switchrole=-1} to assume the Teacher role on course~10 before any assignment tests run.\\
...
TC-003-005..009 & BVA & Grade-to-pass boundaries (0, 0.01, 9.99, 10, 10.01) -- all below max of 100 & success \\ \hline
...
TC-004-013..017 & BVA/UC & Decimal precision edge values + repeat/happy combinations & varies \\ \hline
\end{tabularx}
...
https://xuansang1234.moodlecloud.com/user/editadvanced.php?id=-1,
...
The project description requires \emph{at least two non-functional requirements per student}, with each NFR described in terms of \textbf{testing type, testing approach, and testing tool}. The team identified \textbf{three complementary NFR categories}:
\begin{itemize}
    \item \textbf{Performance} -- load testing with \textbf{Locust} (NFR-1 below).
    \item \textbf{Security} -- passive HTTP probes with the \textbf{requests} library (NFR-2 below). This replaces the original ZAP-based DAST plan with a daemon-free, Python-native approach that produces the same headline signals (CSRF protection, XSS reflection, info-disclosure) without requiring a separate Java process.
    \item \textbf{Accessibility} -- WCAG audit with \textbf{axe-selenium-python} (NFR-3 below).
\end{itemize}

For 5 team members $\times$ 2 NFRs each $=$ 10 NFR slots, the three categories are paired so that every student demonstrates two distinct NFR types on their owned feature (see Table~\ref{tab:per-student-nfr} below). The six NFR files are reorganised so that each file targets one of the six functional features (TC-001 through TC-006), and each file implements two of the three NFR techniques.
...
\textbf{Approach:} Define a virtual user as a Python class deriving from \texttt{HttpUser}; each \texttt{@task} method represents one user interaction (visit login page, submit form, open quiz form, etc.). Locust spawns dozens of these virtual users in parallel and aggregates timing statistics in real time through its web UI on port~8089. To prevent Locust's gevent monkey-patch from racing pytest's selenium imports, the \texttt{HttpUser} subclass is defined inside a \texttt{\_define\_locust\_users()} helper that is only invoked when the module is not loaded under pytest.\\
\textbf{Tool:} Locust 2.43 (\url{https://locust.io}).\\
\textbf{Files:} \texttt{TC-001.py} (Add User), \texttt{TC-002.py} (Create Course), \texttt{TC-004.py} (Grade Student), \texttt{TC-005.py} (Calendar Event).

\subsubsection{NFR-2 -- Security: Passive HTTP Probes via \texttt{requests}}
\textbf{Type:} Security / passive black-box probing of authentication, CSRF protection, input escaping, and error-handling discipline.\\
\textbf{Approach:} A \texttt{requests.Session} is used to (1) confirm anonymous access to protected endpoints is blocked (302, 401, 403, 404, or a redirect to the login form); (2) extract Moodle's \texttt{logintoken} CSRF token and complete a real login, then assert the resulting authenticated session exposes the \texttt{sesskey} CSRF token expected by every form; (3) POST an \texttt{<script>}-based XSS payload as the username and assert the raw script is never echoed back unescaped; (4) GET each endpoint with deliberately malformed query parameters (\texttt{' OR 1=1--}, oversized strings, raw \texttt{<script>} in IDs) and assert the response body contains no PHP error signatures (\texttt{<b>Fatal error</b>}, \texttt{Call to undefined function}, \texttt{Stack trace:}, etc.). This is daemon-free and runs against the live MoodleCloud instance without any prior setup.\\
\textbf{Tool:} \texttt{requests} 2.31 (Python standard ecosystem, no extra service).\\
\textbf{Files:} \texttt{TC-001.py} (Add User), \texttt{TC-003.py} (Create Assignment), \texttt{TC-004.py} (Grade Student), \texttt{TC-006.py} (Quiz Setup).
...
\textbf{Files:} \texttt{TC-002.py} (Create Course), \texttt{TC-003.py} (Create Assignment), \texttt{TC-005.py} (Calendar Event), \texttt{TC-006.py} (Quiz Setup).

\subsection{NFR Coverage Matrix}
Each of the six NFR files combines exactly two of the three NFR techniques (Locust load, \texttt{requests} security, axe accessibility) applied to one of the six functional features. The pairwise coverage of every dimension is therefore demonstrated at least twice across independent feature contexts.
...
\hline
\textbf{File} & \textbf{Feature target} & \textbf{Performance} & \textbf{Security} & \textbf{Accessibility} \\ \hline
\texttt{TC-001.py} & Admin Adds User              & \checkmark (Locust) & \checkmark (\texttt{requests}) & -- \\ \hline
\texttt{TC-002.py} & Admin Creates Course         & \checkmark (Locust) & -- & \checkmark (axe) \\ \hline
\texttt{TC-003.py} & Teacher Creates Assignment   & -- & \checkmark (\texttt{requests}) & \checkmark (axe) \\ \hline
\texttt{TC-004.py} & Teacher Grades Student       & \checkmark (Locust) & \checkmark (\texttt{requests}) & -- \\ \hline
\texttt{TC-005.py} & Admin Creates Calendar Event & \checkmark (Locust) & -- & \checkmark (axe) \\ \hline
\texttt{TC-006.py} & Teacher Sets Up a Quiz       & -- & \checkmark (\texttt{requests}) & \checkmark (axe) \\ \hline
\end{tabularx}
\caption{Pairwise NFR coverage matrix (1 file per functional feature)}
\end{table}
...
Nguyễn Hữu Phúc      & Performance -- Locust load testing       & Security -- \texttt{requests}-based passive probes \\ \hline
Trương Xuân Sang     & Performance -- Locust load testing       & Accessibility -- axe-core WCAG audit \\ \hline
Lâm Minh Tùng Quân   & Security -- \texttt{requests}-based probes & Accessibility -- axe-core WCAG audit \\ \hline
Châu Anh Nhật        & Performance -- Locust load testing       & Accessibility -- axe-core WCAG audit \\ \hline
Trương Gia Kỳ Nam    & Security -- \texttt{requests}-based probes & Accessibility -- axe-core WCAG audit \\ \hline
\end{tabularx}
...
A single PowerShell entry point orchestrates the entire test pipeline (install $\to$ sequenced suite execution). It exposes ten non-interactive modes plus an interactive menu, and includes a dedicated mode for running one TC across all three directories at once:
...
.\run_all.ps1 -Mode nfr          # 6 NFR files (Locust + requests + axe)
...
Removing the OWASP ZAP dependency simplified this script considerably: there is no longer any daemon lifecycle (start/wait/teardown), and the suite runs from a single \texttt{pip install -r requirements.txt} on a clean machine.
...
NFR security probes (\texttt{requests}) & 14 & 14 & 0 & 33 s \\ \hline
NFR accessibility audit (axe-core) & 8 & 8 & 0 & $\sim$4 min \\ \hline
\end{tabularx}
...
\paragraph{Moodle instance migration.}
The original MoodleCloud instance used during Project~\#2 (\url{ihatetesting.moodlecloud.com}) was retired, and the team migrated to a new dedicated instance at \url{xuansang1234.moodlecloud.com}. All hard-coded URLs, credentials, and resource IDs (course~10 for TC-003 assignments, course~12 for TC-006 quizzes, section~39 in both, assignment cmid~41 for the grader) were updated atomically across all six Level~1 source files, six Level~2 CSV files, six NFR files, the cleanup script, and the PowerShell runner.

\paragraph{File-naming convention.}
Mid-project, the team standardised on a \texttt{TC-XXX*} file naming scheme so the relationship between feature, code, and data is obvious from the directory listing. The previous Selenium-friendly names (\texttt{test\_add\_user\_level1.py}, \texttt{test\_data\_tc001.csv}, \texttt{test\_nfr\_01\_login\_perf\_sec.py}) were renamed to \texttt{TC-001\_code.py}, \texttt{TC-001\_data.csv}, and \texttt{TC-001.py} respectively. Since hyphens are not valid Python module identifiers, a repository-level \texttt{pytest.ini} sets \texttt{addopts = --import-mode=importlib} and overrides the \texttt{python\_files} discovery pattern to include \texttt{TC-*.py}.

\paragraph{Locust gevent + pytest conflict.}
Once the security tests started importing \texttt{requests} eagerly, importing Locust (which monkey-patches \texttt{ssl} via gevent at module-load time) after pytest had already loaded selenium triggered a \texttt{RecursionError} during pytest collection. The fix wraps Locust's \texttt{HttpUser} subclass inside a \texttt{\_define\_locust\_users()} helper that is only invoked when the file is loaded outside pytest (\texttt{if "pytest" not in sys.modules}), so the perf class is still defined when Locust runs the file directly but the gevent monkey-patch never executes under pytest.
...
\begin{itemize}
    ...
    \item \textbf{Non-functional methodology} -- integrating three distinct industry-standard tools (Locust, \texttt{requests}, axe-core) into a unified Python testing harness, and documenting each according to the project rubric's \emph{type / approach / tool} triad. Replacing the heavyweight OWASP ZAP DAST dependency with a lightweight \texttt{requests}-based passive scanner kept the security coverage (CSRF token verification, XSS reflection, info-disclosure sentinels, access-control) while eliminating a daemon-lifecycle hazard from the runner script.
